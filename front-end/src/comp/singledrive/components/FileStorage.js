...
import { BorderColor, Folder } from "@mui/icons-material";
...
function FileStorage() {
  ...
  // const [uri, setCopyText] = useState('')
  // const handleCopy = () => {
                ...
                {userFiles.map((fileData, i) => {
                  ...
                  return (
                    <ListItem key={i}>
                      ...
                      <input ref={inputRef} value={uri} type="text" style={{ color:'transparent', border:'none', outline:'none'}} size={90}/>
                    </ListItem>
                  );
                })}
                <Button onClick={copyToClipboard} style={{ right: '300px' }}>Copy</Button>
              </List>
            </div>
          </>
        ) : (
